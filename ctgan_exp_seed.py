...
# This experiment is used to compare ctgan generated samples robustness
...
def prepare_cohort(num_syn_samples, seed, syn_model, df_all, feature_names):
	SEED_VALUE = seed
	...
	with open('results/ctgan_seed_syn_result.txt', 'a') as f:
	...
	return merge, df_test, smd1, len(matched_syn)

# Start the experiments
# Store the results into a dataframe for plotting
df_res = pd.DataFrame(columns=['Seed', 'Matched synthetic data size', 'SMD', 'Qini mean', 'Qini STD','Number of Rules MEAN', 'Number of Rules STD'])

idx = 0


for seed in range(1, 21):
	merge, df_test, smd, num_match = prepare_cohort(500, seed, ctgan_model, df, pre_treatment_var)
	test = df_test.copy()
	test = test_transform_var(test, merge)
	...
	res = {'res_tree':[], 'res_lasso':[], 'num_rules':[]}
	for seed in range(0, 10):
		...
		res['num_rules'].append(num_rules)
		
	with open('results/ctgan_seed_syn_result.txt', 'a') as f:
		f.write('Qini tree:{}\n'.format(np.mean(res['res_tree'])))
		f.write('Qini tree:{}\n'.format(np.mean(res['res_lasso'])))
	df_res.loc[idx]	= [seed, num_match, smd, np.mean(res['res_lasso']), np.std(res['res_lasso']), np.mean(res['num_rules']), np.std(res['num_rules'])]
	idx += 1

df_res.to_csv('results/ctgan_seed_syn_results_df.csv')
