...
def prepare_cohort(num_syn_samples, syn_model, df_all, feature_names):
	SEED_VALUE = 20
	...
	with open('results/tvae_num_syn_result.txt', 'a') as f:
...
df_res = pd.DataFrame(columns=['Number of synthetic samples', 'Matched synthetic data size', 'SMD', 'Qini mean', 'Qini STD','Number of Rules MEAN', 'Number of Rules STD'])

idx = 0


for num in num_samples:
	merge, df_test, smd, num_match = prepare_cohort(num, tvae_model, df, pre_treatment_var)
	test = df_test.copy()
	...
	df_res.loc[idx]	= [num*2, num_match, smd, np.mean(res['res_lasso']), np.std(res['res_lasso']), np.mean(res['num_rules']), np.std(res['num_rules'])]
	idx += 1

df_res.to_csv('results/tvae_num_syn_results_df.csv')
